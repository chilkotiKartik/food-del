...
import { createAction, createSlice, nanoid, combineReducers, configureStore } from '@reduxjs/toolkit';
export { nanoid as itemid } from '@reduxjs/toolkit';
import React, { useState, useCallback, useEffect, createContext, useRef, useMemo, useContext, forwardRef } from 'react';
import { useSelector, useDispatch, Provider, shallowEqual, batch } from 'react-redux';
import _defineProperty from '@babel/runtime/helpers/defineProperty';
import _toConsumableArray from '@babel/runtime/helpers/toConsumableArray';
import produce, { enablePatches, produceWithPatches, original, applyPatches } from 'immer';
...
enablePatches(); // Lazy accumulates changes 
...
function accumulator(reducer) {
  return function (state, action) {
    ...
    var _produceWithPatches = produceWithPatches(present, function (draft) {
    ...
    state.batch.past = state.batch.past || produce(original(state.history.present), function () {});

    (_state$batch$patches = state.batch.patches).push.apply(_state$batch$patches, _toConsumableArray(patches));
...
enablePatches();
var creator = createAction('commitHistory');
function commitHistory$1(state, action) {
  var batch = state.batch;
  var past = batch.past;
  var patches = batch.patches;

  var _produceWithPatches = produceWithPatches(past, function (draft) {
...
var createVersionedSlice = function createVersionedSlice(_ref2) {
  ...
  var extraActions = Object.entries(slice.actions).reduce(function (extraActions, _ref7) {
    ...
    var _typeSplit = _slicedToArray(typeSplit, 2),
        batch = _typeSplit[0],
        type = _typeSplit[1];
  ...
  if (isObject(target) && isObject(source)) {
    Object.keys(source).forEach(function (key) {
      if (isObject(source[key])) {
        if (!(key in target)) Object.assign(target, _defineProperty({}, key, source[key]));else if (isFunction(target[key])) target[key] = function () {
...
var Branch = /*#__PURE__*/function () {
  ...
  _createClass(Branch, [{
    ...
    value: function with_child(child) {
      var children = [].concat(_toConsumableArray(this.children), [child]);
...
var Item = /*#__PURE__*/function () {
  ...
  _createClass(Item, [{
    ...
    value: function with_index() {
      ...
      return new Item(this.detail, this.meta, [].concat(_toConsumableArray(this.indexes), indexes), this.indexes_list);
    }
  }, {
    key: "with_index_list",
    value: function with_index_list() {
      ...
      return new Item(this.detail, this.meta, this.indexes, [].concat(_toConsumableArray(this.indexes_list), indexes));
    }
  }]);
...
function normalizeBranch(branch) {
  ...
  function mergeItemToBranch(branch, itemToMerge) {
    ...
    itemToMerge.index_list.forEach(function (index) {
      ...
      branch.index_list[index] = [].concat(_toConsumableArray(index_list), [itemToMerge.id]);
    });
    return branch;
  }
  ...
  function mergeBranchToBranch(branch, branchToMerge) {
    ...
    keys(branchToMerge.index_list).forEach(function (index) {
      var index_list = branch.index_list[index] || [];
      branch.index_list[index] = [].concat(_toConsumableArray(index_list), _toConsumableArray(branchToMerge.index_list[index]));
    });
    return branch;
  }
...
var getMultipleByFilter = function getMultipleByFilter(state, filter) {
  ...
  return getMultipleById.apply(void 0, [state].concat(_toConsumableArray(ids)));
};
...
var getParentsById = function getParentsById(state, id) {
  ...
  while (parent) {
    parentIds = [].concat(_toConsumableArray(parentIds), [parent.id]);
    parent = getParentNode(state, parent.id);
  }

  return getMultipleById.apply(void 0, [state].concat(_toConsumableArray(parentIds)));
};

var getDirectChildrenById = function getDirectChildrenById(state, id) {
  var node = getNodeById(state, id);
  var childIds = node.childIds;
  return getMultipleById.apply(void 0, [state].concat(_toConsumableArray(childIds)));
};
...
var getChildrenById = function getChildrenById(state, id) {
  ...
  function includeChildren(id) {
    ...
    return node.childIds.reduce(function (children, id) {
      var childrenIds = [].concat(_toConsumableArray(children), [id]);
      return includeChildren(id, childrenIds);
    }, childrenIds);
  }

  return getMultipleById.apply(void 0, [state].concat(_toConsumableArray(childrenIds)));
};
...
var getIndexesForId = function getIndexesForId(state, id) {
  ...
  return [].concat(_toConsumableArray(indexedID), _toConsumableArray(indexedListID)).sort();
};
...
function deleteNode(state, action) {
  ...
  var nodesIds = [].concat(_toConsumableArray(keys(getChildrenById(state, id))), [id]);
...
function deleteMeta(state, action) {
  var id = action.payload.id;
  var nodesIds = [].concat(_toConsumableArray(keys(getChildrenById(state, id))), [id]); // State mutations
...
function deleteIndex(state, action) {
  var id = action.payload.id;
  var ids = [].concat(_toConsumableArray(keys(getChildrenById(state, id))), [id]);
  var nodes = getMultipleById.apply(void 0, [state].concat(_toConsumableArray(ids))); // State mutations
...
var useBatcher = function useBatcher() {
  ...
  var configuration = useSelector(selector);
...
var BuilderContext = /*#__PURE__*/createContext({
  batcher: {
...
var useEvents = function useEvents() {
  var listeners = useRef(new Map());
  var addEventListener = useCallback(function () {
    ...
    var added_listeners = [].concat(_toConsumableArray(type_listeners), [listener]);
    var updated_listeners = Object.assign({}, node_listeners, _defineProperty({}, type, added_listeners));
    listeners.current.set(node, updated_listeners);
  }, []);
  var removeEventListener = useCallback(function () {
    ...
    var updated_listeners = Object.assign({}, node_listeners, _defineProperty({}, type, removed_listeners));
    listeners.current.set(node, updated_listeners);
  }, []);
  var utils = {
...
var DnDEvents = function DnDEvents(_ref) {
  ...
  var transfer = useSelector(selector);
  ...
  var insideOne = useRef(null); // An object to keep reference
  // of current entered elements

  var insideMultiple = useRef([]); // Heavy imperative code below.
  ...
  function getEventElements(event) {
    var _events$getEventClien = events.getEventClientCoords(event),
        _events$getEventClien2 = _slicedToArray(_events$getEventClien, 2),
...
var BuilderProvider = function BuilderProvider(_ref) {
  ...
  return /*#__PURE__*/React.createElement(BuilderContext.Provider, {
    value: context
  }, /*#__PURE__*/React.createElement(DnDEvents, {
    listeners: listeners
  }), children);
};
...
var Builder = function Builder(_ref) {
  ...
  var _useState = useState(initialStore),
      _useState2 = _slicedToArray(_useState, 1),
      store = _useState2[0];

  return /*#__PURE__*/React.createElement(Provider, {
    store: store
  }, /*#__PURE__*/React.createElement(BuilderProvider, null, children));
};
...
var ViewContext = /*#__PURE__*/createContext();
...
var Node = function Node(_ref) {
  var id = _ref.id,
      ...
      rest = _objectWithoutProperties(_ref, ["id", "view", "shallow", "childrenAs", "filterProps"]);
  ...
  return render ? /*#__PURE__*/React.createElement(Resolved, _extends({}, propsAll, {
    id: node.id
  }), !shallow && childIds.length > 0 ? childIds.map(function (childId) {
    return /*#__PURE__*/React.createElement(Child, _extends({
      key: childId,
      id: childId,
      view: view,
...
var Workspace = function Workspace(_ref) {
  ...
  var node = collected.node;
  return node ? /*#__PURE__*/React.createElement(ViewProvider, {
    view: view
...
var Panel = function Panel(_ref) {
  ...
  var Resolved = view[type];
  return Resolved ? /*#__PURE__*/React.createElement(Resolved, {
    id: node.id
  }) : null;
};

var useShareRef = function useShareRef(ref) {
  var innerRef = useRef(ref && ref.current);
  useEffect(function () {
    if (!ref) {
      return;
    }
...
var useBuilderContext = function useBuilderContext() {
  var context = useContext(BuilderContext);
  return context;
};
...
var useEventCallback = function useEventCallback(fn) {
  ...
  callback.current = fn;
  var eventCb = useCallback(function () {
    return callback.current.apply(callback, arguments);
  }, []);
  return eventCb;
};

var DnDSource = /*#__PURE__*/React.forwardRef(function DnDSource(_ref, ref) {
  var onDrop = _ref.onDrop,
      ...
      rest = _objectWithoutProperties(_ref, ["onDrop", "onDragEnd", "onDragOver", "onDragStart", "onDragEnter", "onDragLeave", "onDragIn", "onDragOut", "allowTouch", "dragTouchTimeThreshold", "isDragging", "as"]);

  // Use share ref to both forward
  // ref to parent component and
  // to use ref internally.
  var box = useShareRef(ref);
  var draggingBox = useRef(false);
...
var DnDListener = /*#__PURE__*/React.forwardRef(function DnDListener(_ref, ref) {
  var _ref$listenAny = _ref.listenAny,
      ...
      props = _objectWithoutProperties(_ref, ["listenAny", "listenTransferType"]);
  ...
  return /*#__PURE__*/React.createElement(DnDSource, _extends({}, props, {
    ref: ref,
    isDragging: isDragging
  }));
});

var DnDBuilder = /*#__PURE__*/React.forwardRef(function DnDBuilder(_ref, ref) {
  var props = _extends({}, _ref);

  // Listen to builder
  // drag and drop operations
  // only
  return /*#__PURE__*/React.createElement(DnDListener, _extends({}, props, {
    ref: ref,
...
var DnDBuilderHOC = function DnDBuilderHOC(Component) {
  return /*#__PURE__*/forwardRef(function DnDBuilderHOC(props, ref) {
    return /*#__PURE__*/React.createElement(DnDBuilder, _extends({
      ref: ref,
      as: Component
    }, props));
  });
...
var useMultiCollector = function useMultiCollector(_ref) {
  ...
  var chosen = selector(selectors);
  var nodes = useSelector(chosen, shallowEqual);
  ...
  var metaSelector = function metaSelector(store) {
    return getMultipleMetaById.apply(void 0, [getTree(store)].concat(_toConsumableArray(listIds())));
  };

  var meta = useSelector(metaSelector, shallowEqual);
  ...
  var indexSelector = function indexSelector(store) {
    return getMultipleIndexesForId.apply(void 0, [getTree(store)].concat(_toConsumableArray(listIds())));
  };
...
var useCreator = function useCreator() {
  var dispatch = useDispatch(); // Not creating an unbatched 
...
var useDnD = function useDnD(_ref) {
  ...
  var _useState = useState(initialTransferType),
      _useState2 = _slicedToArray(_useState, 2),
...
var useDnDHelpers = function useDnDHelpers(params) {
  ...
  var transfer = useSelector(transferSelector);
  var events = useEventHelpers();
...
var useEditor = function useEditor(_ref) {
  ...
  dnd.isTransfering;
      ...
      dnd.transferType;
      dnd.setTransferType;
      var dndBag = _objectWithoutProperties(dnd, ["isTransfering", "isTransferingType", "transferType", "setTransferType"]);
...
var useTools = function useTools() {
  ...
  dnd.transferType;
      dnd.setTransferType;
      var bag = _objectWithoutProperties(dnd, ["transferType", "setTransferType"]);
...
var useVersion = function useVersion() {
  var dispatch = useDispatch();
  var history = useSelector(getTreeHistory);
  var context = useBuilderContext();
  ...
  var canRedo = useSelector(redoSelector, shallowEqual);

  var undoSelector = function undoSelector(store) {
    return getCanUndo(getTreeSlice(store));
  };

  var canUndo = useSelector(undoSelector, shallowEqual);
  var historyBag = {
    history: history,
    canUndo: canUndo,
    canRedo: canRedo
...
export { Builder, DnDBuilder, DnDBuilderHOC, DnDListener, Node, Panel, ViewProvider, Workspace, branch, item, useActions, useBuilder, useCollector, useDnDHelpers, useEditor, useMultiCollector, useNodeDnD, useToolDnD, useTools, useView };
